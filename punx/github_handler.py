...
def get_GitHub_credentials(creds_file_name=""):
    ...
    GitHub requests use an access token provided in the local file
    ``__github_creds__.txt`` If found, the file is parsed for ``username
    password token`` or ``token``. The token is assumed to be the last
    whitepace-separated string in the file. The token is unique to a user and
    may be generated by visiting https://github.com/settings/tokens.

    The search for credetials checks these files this order:
        ``creds_file_name``
    ...
    for name in [
        creds_file_name,
        os.path.join(os.path.dirname(__file__), CREDS_FILE_NAME),
        os.path.join(os.path.expanduser("~"), ".config", CREDS_FILE_NAME),
        os.path.join(os.path.expanduser("~"), ".config", "punx", CREDS_FILE_NAME),
    ]:
        if os.path.exists(name):
            return open(name, "r").read().strip().split()[-1]
    return None
