...
def get_GitHub_credentials(creds_file_name=None):
    ...
    ``__github_creds__.txt`` which is placed in the same directory as this
    file. If found, the file is parsed for ``username password token`` or
    ``token``. The token is assumed to be the last whitepace-separated string
    in the file. The token is unique to a user and may be generated by visiting
